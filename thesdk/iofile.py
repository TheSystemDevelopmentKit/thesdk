...
class iofile(IO):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        **kwargs:
        ...
        if parent == None:
            self.print_log(
                type="F", msg="Parent of Verilog input file not given"
            )
        try:
            ...
            self._iotype = kwargs.get(
                "iotype", "sample"
            )  # The file is a data file by default
            # Option: sample, event. file
            # Events are presented in time-value combinatios
            # time in the column 0
            self._ionames = kwargs.get(
                "ionames", []
            )  # list of signal names associated with this io
            self._ioformat = kwargs.get(
                "ioformat", "%d"
            )  # by default, the io values are decimal integer numbers

            self.hasheader = kwargs.get(
                "hasheader", False
            )  # Headers False by default.
        ...
        if hasattr(self.parent, "iofiles"):
            self.print_log(
                type="O",
                msg="Attribute iofiles has been replaced by iofile_bundle",
            )
        ...
        **kwargs:
        ...
        if iotype == "sample":
            ...
            if self.hasheader:
                ...
                df.to_csv(
                    path_or_buf=self.file, sep="\t", index=False, header=False
                )
        # Control file is a different thing
            ...
            for i in range(data.shape[1]):
                if i == 0:
                    if np.iscomplex(data[0, i]) or np.iscomplexobj(data[0, i]):
                        self.print_log(
                            type="F", msg="Timestamp can not be complex."
                        )
                    else:
                        parsed = np.r_["1", data[:, i].reshape(-1, 1)]
            ...
            if self.datatype == "complex" or self.datatype == "scomplex":
                ...
                for i in range(cols):
                    if i == 0:
                        ...
                        self.Data[:, i] = readd.values[:, 2 * i].astype(
                            "int"
