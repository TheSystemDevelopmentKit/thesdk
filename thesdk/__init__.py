...
class thesdk(metaclass=abc.ABCMeta):
        ...
        with open(CONFIGFILE, "r") as fid:
            ...
            for line in fid:
                if re.match(match, line):
                    GLOBALS[name] = reduce(
                        lambda s, func: func(s), func_list, line
                    )
    ...
    def model(self):
        ...
        'py' |  'sv' |  'vhdl' |  'eldo' |  'spectre' | 'ngspice' | 'hw' | 'icarus' | 'ghdl' | 'verilator' | 'ads' | 'emx' | 'xcelium'
    ...
    def model(self, val):
        if val not in [
            ...
            self.print_log(
                type="E", msg="Simulator model %s not supported." % (val)
            )
        self._model = val
        return self._model
    ...
    def simpath(self):
        ...
        self._simpath = "%s/simulations/%s/%s" % (
            self.simpathroot,
            self.model,
            name,
        )
    ...
    def has_lsf(self):
        ...
        if ("LSFINTERACTIVE" not in thesdk.GLOBALS) or (
            ...
            not thesdk.GLOBALS["LSFSUBMISSION"] == ""
        ):
            self._has_lsf = True
        else:
            self._has_lsf = False
        ...
        if len(arg) >= 2:
            ...
            for prop in self.proplist:
                if hasattr(self, prop) and hasattr(self.parent, prop):
                    ...
                    msg = "Setting %s: %s to %s" % (
                        self,
                        prop,
                        getattr(self.parent, prop),
                    )
                    ...
                    msg = (
                        "Property %s not defined for entity %s, omitting copy!"
                        % (prop, obj)
                    )
    ...
    def copy_propval_verbosity(self, value):
        if value not in ["I", "D"]:
            self.print_log(
                type="W",
                msg="Value %s not allowed for copy_propval_verbosity. Use 'I' or 'D'."
                % (value),
            )
        else:
            self.print_log(
                type="W",
                msg="Setting copy_propval_verbosity to '%s'." % (value),
            )
    ...
    def print_log(self, **kwargs):
        ...
        if msgtype == "D":
            ...
            typestr = "[ERROR]"
            msg = (
                "Incorrect message type '%s'. Choose one of 'D', 'I', 'E' or 'F'."
                % type
            )
            print(
                "%s %s%s%s %s: %s"
    ...
    def silence(self, show_error=True, debug=False):
        ...
        if not debug:
            with open(os.devnull, "w") as fnull:
                if not show_error:
                    with cl.redirect_stderr(fnull) as err, cl.redirect_stdout(
                        fnull
                    ) as out:
                        yield (err, out)
                else:
    ...
    def run_parallel(self, **kwargs):
        ...
        for j in range(nbatch):
            ...
            for i in dutrange:
                self.print_log(
                    type="I",
                    msg="Starting parallel run %d/%d" % (i + 1, len(duts)),
                )
                que.append(multiprocessing.Queue())
                proc.append(
                    multiprocessing.Process(target=getattr(duts[i], method))
                )
                duts[i].par = True
                duts[i].queue = que[-1]
                proc[-1].start()
            ...
            for i in dutrange:
                ...
                if ret_dict:
                    self.print_log(
                        type="I",
                        msg="Saving results from parallel run of %s"
                        % (duts[i]),
                    )
                    ...
                    self.print_log(
                        type="W",
                        msg="Parallel run %d/%d failed (with name: %s). Returned dict was empty!"
                        % (i + 1, len(duts), name),
                    )
                proc[n].join()
                n += 1
    ...
    def netlist_params(self):
        ...
        [TODO] This creates a link to something (ecd_methods) that is not in open domain. Refine.
        """
    ...
    def _write_state(self):
        ...
        try:
            ...
            self.print_log(
                type="E", msg="Failed saving state to %s" % self.statedir
            )
    ...
    def _read_state(self):
        ...
        if not os.path.exists(pathname):
            self.print_log(
                type="E", msg="Existing results not found in %s" % pathname
            )
        ...
        try:
                ...
                for name, val in obj.__dict__.items():
                    ...
                    if name == "_IOS" and type(val).__name__ == "Bundle":
                        for ioname, ioval in val.Members.items():
                            self.print_log(
                                type="D",
                                msg="Assigning data to %s at %s"
                                % (
                                    ioname,
                                    hex(
                                        id(self.__dict__[name].Members[ioname])
                                    ),
                                ),
                            )
            ...
            self.print_log(
                type="F", msg="Failed loading state from %s" % pathname
            )
    ...
    def delete_iofile_bundle(self):
        ...
        for name, val in self.iofile_bundle.Members.items():
            if self.preserve_iofiles:
                self.print_log(
                    type="I", msg="Preserving iofiles for %s" % (name)
                )
            else:
                if val.preserve:
                    # In case preserve flag is set by other means
                    self.print_log(
                        type="I", msg="Preserve_value is %s" % (val.preserve)
                    )
                    self.print_log(
                        type="I", msg="Preserving file %s" % (val.file)
                    )
                else:
                    val.remove()
